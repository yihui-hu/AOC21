...
int main()
{
    ...
    for (int i = 0; i < lines.size(); i++) {
        string line = lines[i];
        bool valid = true; // to check if closing bracket is valid, if not break out of loop immediately and return offender or do smth else
        long sum = 0;
        ...
        for (int j = 0; j < line.length(); j++) {
            ...
            if (next == r_c or 
                next == c_c or 
                next == s_c or 
                next == a_c) { // if it is a closing bracket, check if it corresponds with the opening bracket at front of deque
                    
                if (curr == r_o and next != r_c) {
                    errors.push_back(next); // for part 1 tracking
                    valid = false; // if it does not match, return false so it breaks out of the loop. this line is donezo, move onto the next
        ...
        if (sum > 0) { // for lines that are complete or have an offending closing bracket, sum will be 0, so check for anything more than 0
